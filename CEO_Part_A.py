'''
Put this code in the appendix of the lab report!!
'''
...
x = []
y = []
with open("CEO_Part_A1.csv", "r", encoding = "utf-8") as infile:
    while True:
        row = infile.readline()
        if row == "":
            break
...
x2 = np.arange(0, 0.0012, 0.0000001)

for i in range(len(x2)):
    plt.plot(x2[i], a * np.cos(b * x2[i] + c) * np.e ** (-d * x2[i]), "o", color = "red", ms = 0.2)
